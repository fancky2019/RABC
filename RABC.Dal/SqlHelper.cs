...
namespace RABC.Dal
{
    ...
    public sealed class SqlHelper
    {
        ...
        {
            return ConfigurationManager.ConnectionStrings["RABCConnectionString"].ConnectionString;
            //return ConfigurationManager.AppSettings["RABCConnectionString"].ToString();
        }
        /// <summary> 
        /// 一个有效的数据库连接对象 
        /// </summary> 
        ...
        public static System.Xml.XmlReader ExecuteXmlReader(SqlConnection connection, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            ...
            try
            {
                ...
                // 执行命令 
                System.Xml.XmlReader retval = cmd.ExecuteXmlReader();
        ...
        /// <param name="parameterValues">分配给存储过程输入参数的对象数组</param> 
        /// <returns>返回XmlReader结果集对象.</returns> 
        public static System.Xml.XmlReader ExecuteXmlReader(SqlConnection connection, string spName, params object[] parameterValues)
        {
            if (connection == null) throw new ArgumentNullException("connection");
            if (spName == null || spName.Length == 0) throw new ArgumentNullException("spName");
        ...
        /// <param name="parameterValues">分配给存储过程输入参数的对象数组</param> 
        /// <returns>返回一个包含结果集的DataSet.</returns> 
        public static System.Xml.XmlReader ExecuteXmlReader(SqlTransaction transaction, string spName, params object[] parameterValues)
        {
            if (transaction == null) throw new ArgumentNullException("transaction");
            if (transaction != null && transaction.Connection == null) throw new ArgumentException("The transaction was rollbacked or commited, please provide an open transaction.", "transaction");
            if (spName == null || spName.Length == 0) throw new ArgumentNullException("spName");
        ...
        /// <param name="dataRow">使用DataRow作为参数值</param> 
        /// <returns>返回XmlReader结果集对象.</returns> 
        public static System.Xml.XmlReader ExecuteXmlReaderTypedParams(SqlConnection connection, String spName, DataRow dataRow)
        {
            if (connection == null) throw new ArgumentNullException("connection");
            if (spName == null || spName.Length == 0) throw new ArgumentNullException("spName");
        ...
        /// <param name="dataRow">使用DataRow作为参数值</param> 
        /// <returns>返回XmlReader结果集对象.</returns> 
        public static System.Xml.XmlReader ExecuteXmlReaderTypedParams(SqlTransaction transaction, String spName, DataRow dataRow)
        {
            if (transaction == null) throw new ArgumentNullException("transaction");
            if (transaction != null && transaction.Connection == null) throw new ArgumentException("The transaction was rollbacked or commited, please provide an open transaction.", "transaction");
            if (spName == null || spName.Length == 0) throw new ArgumentNullException("spName");
        ...
        /// 存储过程分页读取数据
        /// </summary>
        /// <param name="showField">分页总数</param>
        /// <param name="showField">记录总数</param>
        ///  <param name="showField">要显示的字段</param>
        /// <param name="selectField">要查询的字段</param>
        /// <param name="tableName">用于查询数据的表名称</param>
        /// <param name="countTable">用于分页的表名称</param>
        /// <param name="conditions">查询条件</param>
        /// <param name="orderBy">排序字段</param>
        /// <param name="pageIndex">当前页</param>
        /// <param name="pageSize">每页显示</param>
        /// <returns></returns>
        public static DataTable GetDataTableByPage(out int pageCount, out int recordCount, string showField, string selectField, string tableName, string countTable, string conditions = null, string orderBy = null, int pageIndex = 1, int pageSize = 30)
        {
            ...
            try
            {
                //string sqlStart = ((pageIndex - 1)*pageSize + 1).ToString();
                //索引是从0开始
                string sqlStart;
                if (pageIndex == 1)
                {
                ...
                {
                    sqlStart = ((pageIndex - 1) * pageSize).ToString();
                    sqlStart = (int.Parse(sqlStart) + 1).ToString();
                }
                ...
                string sqlQuery = string.Format("SELECT {0} FROM (SELECT ROW_NUMBER() OVER(ORDER BY {1}) AS RowIndex, " + selectField + " FROM {2} {3}) AS PageTable WHERE RowIndex BETWEEN {4} AND {5};"
                       , showField, orderBy, tableName, conditions, sqlStart, sqlEnd);
                sqlQuery += string.Format("SELECT COUNT(0) FROM {0} {1}", countTable, conditions);

                DataSet ds = ExecuteDataset(GetConnSting(), CommandType.Text, sqlQuery, (SqlParameter[])null);
                dt = ds.Tables[0];
                ...
                {
                    pageCount = recordCount / pageSize + 1;// --总页数就等于总记录条数除以页大小加1
                }
            }
            catch (Exception ex)
            {
        ...
        /// <param name="pageSize">每页显示</param>
        /// <returns></returns>
        public static DataTable GetDataTableByPage(out int pageCount, out int recordCount, string showField, string selectField, string tableName, string countTable, string conditions = null, string groupBy = null, string orderBy = null, int pageIndex = 1, int pageSize = 30)
        {
            DataTable dt = new DataTable();
            pageCount = 0;
            recordCount = 0;
            try
            {
                ...
                string sqlQuery = string.Format("SELECT {0} FROM (SELECT ROW_NUMBER() OVER(ORDER BY {1}) AS RowIndex, " + selectField + " FROM {2} {3} {4}) AS PageTable WHERE RowIndex BETWEEN {5} AND {6};"
                       , showField, orderBy, tableName, conditions, groupBy, sqlStart, sqlEnd);
                sqlQuery += string.Format("SELECT COUNT(0) FROM ({0} {1} {2}) as d", countTable, conditions, groupBy);
        ...
        {
            DataSet ds = SqlHelper.ExecuteDataset(GetConnSting(), commandType, commandText);
            return ds.Tables[0];
        }
    }
